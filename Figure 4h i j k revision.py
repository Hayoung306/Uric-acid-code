            ...
            for params in ParameterGrid(param_grid):
                ...
                for inner_train_idx, inner_val_idx in current_splitter:
                    ...
                    if scale_features:
                        ...
                        X_inner_train_scaled = pd.DataFrame(
                            scaler.fit_transform(X_inner_train),
                            columns=X_inner_train.columns
                        )
                        X_inner_val_scaled = pd.DataFrame(
                            scaler.transform(X_inner_val),
                            columns=X_inner_val.columns
                        )
                    else:
                        X_inner_train_scaled = X_inner_train.copy()
                        X_inner_val_scaled = X_inner_val.copy()
                    ...
                    y_val_prob = model.predict_proba(X_inner_val_scaled)[:, 1]
